  ...
  return (
    <>
      <h1 className='font-bold'>Products</h1>
        ...
        {
            Highlight && Stationary.filter((item)=>item.price<100).map((item)=>{
                return <li>{item.name} - {item.price} - {item.quantity}</li>
            })
        }
      <button className='bg-slate-600 text-white p-2 rounded-sm' onClick={handlehighlight}>Filter with Price</button>
    </>
  )
}
